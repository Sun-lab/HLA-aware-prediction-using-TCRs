...
def read_in_data(data_path):
    ...
    HLA_index_mat = pd.read_csv( os.path.join(data_path, "HLA_II_neighbor_index_mat.csv"))
    HLA_index_mat = drop_df_nas(HLA_index_mat, 1)
    neigbor_weights_mat = pd.read_csv( os.path.join(data_path, "HLA_II_weights_mat.csv"))
    neigbor_weights_mat = drop_df_nas(neigbor_weights_mat, 0)
    AA_index = np.array(pd.read_csv(os.path.join(data_path, "HLA_II_index.csv"),header=None,dtype=int )[0])

    new_HLA = np.vstack((HLA, HLA[exception_index]))


    return TCR, new_HLA, CMV, train_index, test_index, HLA_index_mat, neigbor_weights_mat, AA_index
...
def work_for_HLA_ii(HLA_index_mat, neigbor_weights_mat, HLA, TCR, HLA_index, CMV, log_rd, train_index, test_index, 
                    HLA_II_index, pval_path, auc_path):
    target_HLA = HLA_II_index[HLA_index]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hla_id', type=int, help='the relative index of HLA (among HLA-IIs)')
    ...
    TCR, HLA, CMV, train_index, test_index, HLA_index_mat, neigbor_weights_mat, HLA_II_index = read_in_data(data_path)
    log_rd = np.log(np.sum(TCR,0))
  
    work_for_HLA_ii(HLA_index_mat, neigbor_weights_mat, HLA, TCR, hla_id, CMV, log_rd,
                    train_index, test_index, HLA_II_index, pval_path, auc_path)
