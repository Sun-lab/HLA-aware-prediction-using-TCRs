...
def predict_test_individual(test_index, sign_dict, base_tcr_pool, HLA, TCR):
    pred_probs = []
    
    test_index_list = test_index.tolist()
    for ind_index in test_index_list:
        existing_hlas = np.where(HLA[:,ind_index]==1)[0]
        ind_sign_union = base_tcr_pool.tolist()
...
if __name__ == "__main__":
    ...
    pred_probs = predict_test_individual(test_index, sign_dict, base_tcr_pool, HLA, TCR)

    Auc = metrics.roc_auc_score( CMV[test_index ], pred_probs )

    auc_filename = os.path.join(res_path, "union_all_HLAs_with_base_auc.csv")
