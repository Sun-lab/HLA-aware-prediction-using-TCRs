...
p1 = ggplot(df1, aes(x = agnostic_AUC, y = knn_AUC, size = size_category, color = HLA_group)) +
...
p2 = ggplot(df2, aes(x = agnostic_AUC, y = knn_AUC, size = size_category, color = HLA_group)) +
  geom_point(alpha = 0.6, show.legend = T) +
  scale_size_manual(
    name = "Freq",
    values = c("<=70" = 2, ">70" = 3),  # adjust sizes as needed
...
pdf("supplementary_figure3_panels/Supplementary_Fig3.pdf", width = 5.2, height = 3.2)
grid.arrange(
  arrangeGrob(
    p1, p2,
    nrow=1
